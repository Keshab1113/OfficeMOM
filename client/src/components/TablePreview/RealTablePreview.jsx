...
const DownloadOptions = ({ onChange }) => {
  ...
  const handleChange = (format) => {
    const newOptions = { ...options, [format]: !options[format] };
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        <label className="flex items-center space-x-2">
          <input
            type="checkbox"
            checked={options.word}
            onChange={() => handleChange('word')}
            className="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"
          />
          <span className="text-sm text-gray-700 dark:text-gray-300">Word Document</span>
        </label>
        <label className="flex items-center space-x-2">
          <input
            type="checkbox"
            checked={options.excel}
            onChange={() => handleChange('excel')}
            className="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"
          />
          <span className="text-sm text-gray-700 dark:text-gray-300">Excel Spreadsheet</span>
        </label>
      </div>
    </div>
  );
...
const RealTablePreview = ({ showFullData = [], detectLanguage = 'en', onSaveTable }) => {
  ...
  const handleMouseMove = useCallback((e) => {
    ...
    if (type === 'column') {
      ...
      setColumnWidths(prev => ({
        ...prev,
        [index]: Math.max(120, (prev[index] || 200) + deltaX)
      }));
    } else if (type === 'row') {
  ...
  const getColumnWidth = (index) => columnWidths[index] || 200;
  const getRowHeight = (index) => rowHeights[index] || 64;

  return (
    <section className="my-20 md:my-4 bg-amber-50 dark:bg-gray-800 rounded-2xl w-full overflow-hidden max-w-[70vw]">
      <div className="block overflow-hidden rounded-xl border border-gray-200 dark:border-gray-700 shadow-2xl bg-white dark:bg-gray-800 w-full max-w-screen">
        <div className="overflow-auto max-h-[71vh] hidescrollbar">
          <table className="w-full border-collapse table-fixed">
            <thead>
              <tr className="bg-gradient-to-r sticky top-0 z-20 from-blue-50 via-indigo-50 to-purple-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 border-b-2 border-gray-200 dark:border-gray-600">
                ...
                {(translatedColumns.length > 0 ? translatedColumns : allColumns).map((col, index) => (
                  <th
                    key={col}
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
              {tableData?.map((row, rowIndex) => (
                ...
                >
                  <td className="px-3 sm:px-8 py-3 sm:py-6 text-center border-r border-gray-100 dark:border-gray-700" style={{ width: '100px' }}>
                  ...
                  {allColumns.map((col, colIndex) => {
                    ...
                    return (
                      <td
                        key={col}
                        className={`border-x border-gray-100 dark:border-gray-700 transition-all duration-300 relative overflow-hidden ${
                          isEditing
                            ? ""
                  ...
                  <td className="px-3 sm:px-8 py-3 sm:py-6 text-center border-r border-gray-100 dark:border-gray-700" style={{ width: '120px' }}>
                    <div className="flex justify-center">
                      <button
                  ...
                  <div 
                    className="absolute bottom-0 left-0 right-0 h-1 cursor-row-resize opacity-0 group-hover:opacity-100 bg-green-400 hover:bg-green-600 transition-all duration-200"
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-2 sm:space-y-0">
            <div className="text-center sm:text-left">
              <span className="hidden sm:inline">
                Click any cell to edit • Press Ctrl+Enter to save • Press Escape to cancel • Hover borders to resize
              </span>
