          ...
          <ul className="space-y-3">
            {reqs.map(r => (
              <li key={r.socketId} className="border border-gray-200 rounded-lg p-3 hover:bg-gray-50">
                <div className="font-medium text-gray-800">{r.deviceName || 'Guest'}</div>
                <div className="text-xs text-gray-500 mt-1">{r.deviceLabel || 'Unknown device'}</div>
                <div className="mt-3 flex gap-2">
                  <button 
                    className="px-3 py-2 cursor-pointer bg-green-600 text-white rounded flex items-center gap-2 hover:bg-green-700 flex-1 justify-center"
                    onClick={() => onApprove(r.socketId)}
                  >
                    <FaCheck /> Admit
                  </button>
                  <button 
                    className="px-3 py-2 cursor-pointer bg-red-600 text-white rounded flex items-center gap-2 hover:bg-red-700 flex-1 justify-center"
